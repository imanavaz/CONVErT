<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template match="HorizontalBar">
    <sales>
      <xsl:variable name="arg37" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
        <xsl:value-of select="Value" />
      </xsl:variable>
      <xsl:variable name="output35" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
        <xsl:value-of select="$arg37 div 2" />
      </xsl:variable>
      <xsl:variable name="output36" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
        <xsl:value-of select="$arg37 div 2" />
      </xsl:variable>
      ...
      <Amount1>
        <xsl:copy-of select="$output35" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" />
      </Amount1>
      <Amount2>
        <xsl:copy-of select="$output36" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" />
      </Amount2>
    </sales>
  </xsl:template>
</xsl:stylesheet>